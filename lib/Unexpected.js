...
var splitShiftedAssertions = require('./splitShiftedAssertions');
...
function thenResult(resolve, reject) {
    var chainingContext = this._chainingContext;
    if (chainingContext.promise) {

        return chainingContext.promise.then(function (result) {
            if (!chainingContext.calledAssertions[chainingContext.calledAssertions.length - 1].shiftNext &&
                result !== undefined) {
                chainingContext.subject = result;
            }
            resolve(chainingContext.subject);
        }, reject);
    }
    return resolve(this._chainingContext.subject);
}

function forwardFlags() {
    this._chainingContext.forwardFlags = this._chainingContext.parentFlags;
    return this;
}
...
function Unexpected(options) {
    ...
    this.assertionMethods = options.assertionMethods || {};
    ...
    this._outputFormat = options.format || magicpen.defaultFormat;
    this.assertionRoot = { then: thenResult, forwardFlags: forwardFlags };

    var that = this;
    // Regenerate the assertionRoot for chained assertions, bound to this instance
    Object.keys(this.assertionMethods).reduce(function (assertionRoot, assertionMethod) {
        assertionRoot[assertionMethod] = wrapAssertions(that, assertionMethod);
        return assertionRoot;
    }, this.assertionRoot);

    this.installedPlugins = options.installedPlugins || [];

    // Make bound versions of these helpers up front to save a bit when creating wrapped expects:
...
Unexpected.prototype.addAssertion = function (patternOrPatterns, handler, childUnexpected) {
    ...
    var assertionMethods = this.assertionMethods;
    var assertionRoot = this.assertionRoot;
    ...
    assertionHandlers.forEach(function (handler) {
        ...
        var assertionMethodName = camelCase(handler.testDescriptionString);
        var assertionHandlers = assertions[handler.testDescriptionString];
        var assertionMethodHandlers = assertionMethods[assertionMethodName];
        ...
        if (!assertionHandlers) {
            ...
            assertionMethods[assertionMethodName] = [handler];
        } else {
            var i = 0;
            ...
            assertionMethodHandlers.splice(i, 0, handler);
        }

        if (assertionRoot[assertionMethodName] === undefined) {
            assertionRoot[assertionMethodName] = wrapAssertions(that, assertionMethodName);
        }
    });
...
function installExpectMethods(unexpected) {
    ...
    expect.assertionMethods = unexpected.assertionMethods;
...
Unexpected.prototype.throwAssertionMethodNotFoundError = function (subject, testMethodName, args) {
    return this.throwAssertionNotFoundError(subject, testMethodName, 'assertionMethods', args);
};

Unexpected.prototype.throwAssertionStringNotFoundError = function (subject, testDescriptionString, args) {
    return this.throwAssertionNotFoundError(subject, testDescriptionString, 'assertions', args);
};

Unexpected.prototype.throwAssertionNotFoundError = function (subject, testName, assertionCollectionName, args) {
    var candidateHandlers = this[assertionCollectionName][testName];
    var that = this;
    if (candidateHandlers) {
        this.fail({
            message: function (output) {
                ...
                output.append(createStandardErrorMessage(output.clone(), subjectOutput, testName, argsOutput)).nl()
                    .indentLines();
                output.i().error('The assertion does not have a matching signature for:').nl()
                    .indentLines().i().text('<').text(that.findTypeOf(subject).name).text('>').sp().text(testName);
    ...
    var instance = this;
    while (instance) {
        Array.prototype.push.apply(assertionStrings, Object.keys(instance[assertionCollectionName]));
        instance = instance.parent;
    }

    function compareAssertions(a, b) {
        var aAssertion = that.lookupAssertionRule(subject, a, 'assertions', args);
        var bAssertion = that.lookupAssertionRule(subject, b, 'assertions', args);
    ...
    assertionStrings.forEach(function (assertionString) {
        var score = leven(testName, assertionString);
    ...
    this.fail({
        ...
        message: function (output) {
            output.error("Unknown assertion '").jsString(testName)
                .error("', did you mean: '").jsString(bestMatch.assertion).error("'");
        }
    });
};

Unexpected.prototype.lookupMethodAssertionRule = function (subject, testMethod, args, requireAssertionSuffix) {
    return this.lookupAssertionRule(subject, testMethod, 'assertionMethods', args, requireAssertionSuffix, true);
}

Unexpected.prototype.lookupAssertionRule = function (subject, testDescriptionString, assertionCollectionName, args,
                                                     requireAssertionSuffix, ignoreTrailingAssertions) {
    var that = this;
    ...
    var instance = this;
    while (instance) {
        var instanceHandlers = instance[assertionCollectionName][testDescriptionString];
        if (instanceHandlers) {
    ...
    function removeTrailingAssertions(handlerArgs) {
        return handlerArgs.filter(function (arg) { return arg.type.name !== 'assertion'; })
    }
    ...
    function matchesHandler(handler, relaxed) {
        ...
        var handlerArgs = ignoreTrailingAssertions ? removeTrailingAssertions(handler.args) : handler.args;

        var requireArgumentsLength = calculateLimits(handlerArgs);
        ...
        var lastRequirement = handlerArgs[handlerArgs.length - 1];
        return args.every(function (arg, i) {
            if (i < handlerArgs.length - 1) {
                return matches(arg, handlerArgs[i].type, i, relaxed);
            } else {
...
Unexpected.prototype._expect = function expect(context, args) {
    ...
    if (args.length < 1) {
        throw new Error('The expect function requires at least one parameter for the chained syntax or two parameters for the string syntax.');
    } else if (args.length === 1) {
        return this._expectChained(context, subject);
    ...
    try {
        var result = executeExpect(that, context, subject, testDescriptionString, Array.prototype.slice.call(args, 2));
...
function executeExpect(unexpected, context, subject, testDescriptionString, args) {
    var that = unexpected;
    var assertionRule = that.lookupAssertionRule(subject, testDescriptionString, 'assertions', args);
    ...
    if (!assertionRule) {
        ...
        for (var n = tokens.length - 1; n > 0; n -= 1) {
            ...
            assertionRule = that.lookupAssertionRule(subject, prefix, 'assertions', argsWithAssertionPrepended, true);
            if (assertionRule) {
        ...
        if (!assertionRule) {
            that.throwAssertionStringNotFoundError(subject, testDescriptionString, args);
        }
    }
    ...
    var wrappedExpect = createWrappedExpect(unexpected, context, assertionRule, flags, subject, args)
...
Unexpected.prototype.clone = function () {
    ...
    var clonedAssertionMethods = {};
    Object.keys(this.assertionMethods).forEach(function (assertion) {
        clonedAssertionMethods[assertion] = [].concat(this.assertionMethods[assertion]);
    }, this);

    var unexpected = new Unexpected({
        assertions: clonedAssertions,
        assertionMethods: clonedAssertionMethods,
...
function wrapAssertions(instance, name) {

    var wrapped = function () {

        var that = this;
        var args = Array.prototype.slice.call(arguments);
        if (this._chainingContext.promise) {
            this._chainingContext.promise = this._chainingContext.promise.then(function () {
                return callChainingAssertion.call(that, instance, that._chainingContext.expectContext, name, args);
            });
            return this;
        }

        var result;
        result = callChainingAssertion.call(that, instance, that._chainingContext.expectContext, name, args);
        if (isPromiseLike(result)) {
            this._chainingContext.promise = result;
        }
        return this;
    };
    return wrapped;
...
function callChainingAssertion(unexpected, context, name, args) {
    var that = this;
    var subjectType = this._chainingContext.subjectType;
    var subject = this._chainingContext.subject;
    var assertionRule = unexpected.lookupMethodAssertionRule(this._chainingContext.subject, name, args, false);
    if (!assertionRule) {
        unexpected.throwAssertionMethodNotFoundError(subject, name, args);
    }
    var calledAssertionEntry = {
    ...
    this._chainingContext.calledAssertions.push(calledAssertionEntry);

    var flags = extend({}, assertionRule.flags);
    if (this._chainingContext.forwardFlags) {
        var forwardFlags = this._chainingContext.forwardFlags;
        Object.keys(this._chainingContext.forwardFlags).forEach(function (flagName) {
            flags[flagName] = forwardFlags[flagName];
        })
    }


    var chainingContext = this._chainingContext;
    chainingContext.errorMode = null;
    var wrappedExpect = createWrappedExpect(unexpected, context, assertionRule, flags, subject, args);
    wrappedExpect.fail = function () {
        try {
            Unexpected.prototype.fail.apply(context.expect, arguments)
        } catch (e) {
            // The errorMode on the calledAssertionEntry will be set by the handler to the
            // nextErrorMode in the chainingContext, so set it here so the entry
            // can be set correctly
            chainingContext.nextErrorMode = wrappedExpect.errorMode;

            var dummyExpect = Object.create(wrappedExpect);
            dummyExpect.errorMode = 'bubble';
            dummyExpect.testDescription = null;
            throw new UnexpectedError(dummyExpect, e)
        }
    };


    wrappedExpect.shift = function shift(newSubject) {
        chainingContext.nextErrorMode = wrappedExpect.errorMode;
        chainingContext.shiftNext = true;
        if (newSubject !== undefined && arguments.length > 0) {
            chainingContext.subject = newSubject;
            chainingContext.subjectType = unexpected.findTypeOf(newSubject);
        }
    };

    wrappedExpect.standardErrorMessage = chainingStandardErrorMessage.bind(null, that._chainingContext);

    // Clear the nextErrorMode, if the assertion handler calls shift after setting the mode
    chainingContext.nextErrorMode = null;
    chainingContext.shiftNext = false;

    var result;
    try {
        result = oathbreaker(assertionRule.handler.apply(wrappedExpect, [wrappedExpect, subject].concat(args)));
        calledAssertionEntry.shiftNext = chainingContext.shiftNext;
        calledAssertionEntry.errorMode = chainingContext.nextErrorMode;
        calledAssertionEntry.subjectOutput = wrappedExpect.subjectOutput;
        calledAssertionEntry.argsOutput = wrappedExpect.argsOutput;
        if (isPendingPromise(result)) {
            unexpected.expect.notifyPendingPromise(result);
            result = result.then(undefined, function (e) {
                if (e && e._isUnexpected ) {
                    e = splitShiftedAssertions(wrappedExpect, chainingContext.calledAssertions, e);
                    // TODO: not sure what the context.level has to do with setting the error message
                    if (context.level === 0) {
                        unexpected.setErrorMessage(e);
                    }
                }
        ...
        if (e._isUnexpected) {
            var wrappedError = splitShiftedAssertions(wrappedExpect, chainingContext.calledAssertions, e)
            unexpected.setErrorMessage(wrappedError);
            throw wrappedError;
        }
        throw e;
    }
...
function createWrappedExpect(unexpected, context, assertionRule, flags, subject, args) {
    ...
    var wrappedExpect = function (subject, testDescriptionString) {
        if (arguments.length === 0) {
            ...
            return unexpected._expectChained(context, subject, flags);
        } else if (testDescriptionString && testDescriptionString._expectIt) {
        ...
        return wrappedExpect.callInNestedContext(function () {
            return executeExpect(unexpected, context.child(), subject, testDescriptionString, args);
        });
    };

    utils.setPrototypeOfOrExtend(wrappedExpect, unexpected._wrappedExpectProto);
    ...
    wrappedExpect.testDescription = assertionRule.testDescriptionString;
...
Unexpected.prototype._expectChained = function (context, subject, parentFlags) {
    const rootWithState = Object.create(this.assertionRoot, {
        _chainingContext: {
            value: {
                subject: subject,
                subjectType: this.findTypeOf(subject),
                calledAssertions: [],
