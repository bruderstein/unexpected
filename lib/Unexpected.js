...
function Unexpected(options) {
    ...
    this._outputFormat = options.format || magicpen.defaultFormat;
    this.assertionRoot = { then: thenResult };
...
Unexpected.prototype._expect = function expect(context, args) {
    ...
    if (args.length < 1) {
        ...
        // TODO: find out if we need to pass context here
...
function callAssertion(unexpected, context, name, args) {
    ...
    // wrappedExpect.testDescription = assertion.testDescriptionString;
    ...
    var result;
    try {
        result = assertionRule.handler.apply({ flags: assertionRule.flags || {} }, [wrappedExpect, subject].concat(args));
    } catch (e) {
        if (e._isUnexpected) {
...
Unexpected.prototype._expectChained = function (context, subject) {
    var that = this;
...
Unexpected.prototype.createStandardErrorMessageFromChain = function (output, calledAssertions, options) {
    ...
    var outputs = calledAssertions.reduce(function (outputs, assertion) {
        ...
        if (useSubject) {
            ...
            if (subject) {
                // TODO: we almost certainly need to walk the type hierarchy to find the inspect method
                // TODO: depth here?
                subjectOutput.appendInspected(subject); // assertion.subjectType.inspect(assertion.subject, 0, subjectOutput, that.inspect.bind(that));
            }
            useSubject = false;
        }
