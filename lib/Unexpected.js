...
function thenResult(resolve, reject) {
    if (this._context.promise) {
        return this._context.promise.then(resolve, reject);
    }
...
function callAssertion(unexpected, context, name, args) {
    ...
    this._context.calledAssertions.push({
    ...
    var wrappedExpect = function (subject, testDescriptionString) {
        if (arguments.length === 0) {
            ...
            // TODO: ask people what this is used for? - nested expect with one argument, is that like expect.shift()?
            // return addAdditionalPromiseMethods(makePromise.resolve(subject), wrappedExpect, subject);
    ...
    wrappedExpect.standardErrorMessage = function (output, options) {
        return unexpected.standardErrorMessage(that._context, output, options);
    };

    var result;
    try {
        result = assertionRule.handler.apply(wrappedExpect, [wrappedExpect, subject].concat(args));
    } catch (e) {
        if (e._isUnexpected) {
            var wrappedError = new UnexpectedError(wrappedExpect, e);
...
Unexpected.prototype._expectChained = function (context, subject, parentFlags) {
    ...
    this.expect.shift = (function shift(newSubject) {
        this._context.subject = newSubject;
        this._context.subjectType = that.findTypeOf(newSubject);
    }).bind(rootWithState);

    return rootWithState;
};

Unexpected.prototype.createStandardErrorMessageFromChain = function (output, calledAssertions, options) {
...
//    .sp().append(subjectOutput).sp().error(calledAssertions[0].assertion.testDescriptionString);
    // if (argsOutput.size().width > 0) {
//    output.sp().append(argsOutput);
//  }
    /*
...
Unexpected.prototype.standardErrorMessage = function (context, output, options) {
    var that = this;
    ...
    return this.createStandardErrorMessageFromChain(output, context.calledAssertions, options);
};
