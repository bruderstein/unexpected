...
Unexpected.prototype._expect = function expect(context, args) {
    ...
    if (args.length < 1) {
        ...
        return this._expectChained(subject);
    ...
    var wrapped = function () {
        ...
        if (this._context.promise) {
            this._context.promise = this._context.promise.then(function () {
                    return callAssertion.call(that, instance, name, args);
            });
            return this;
        }

        var result;
        result = callAssertion.call(that, instance, name, args);
...
function callAssertion(unexpected, name, args) {
    var subjectType = this._context.subjectType;
    var subject = this._context.subject;
    var assertion = unexpected.lookupAssertionRule(this._context.subject, name, 'assertionMethods', args);
    if (!assertion) {
    ...
    // TODO: installExpectMethods is probably wrong here - i think we only need a createWrappedExpectProto thing
    var wrappedExpect = installExpectMethods(unexpected, Unexpected.prototype.expect);
    ...
    wrappedExpect.shift = function shift(newSubject) {
        if (newSubject !== undefined && arguments.length > 0) {
            context.subject = newSubject;
            context.subjectType = unexpected.findTypeOf(newSubject);
        }
    };
    ...
    var result;
    try {
        result = assertion.handler.apply({ flags: assertion.flags || {} }, [wrappedExpect, subject].concat(args));
    } catch (e) {
        if (e._isUnexpected) {
...
Unexpected.prototype._expectChained = function (subject) {
    var that = this;
    ...
    rootWithState._context.expectContext = this.expect;
    ...
    var outputs = calledAssertions.reduce(function (outputs, assertion) {
        ...
        if (useSubject) {
            ...
            if (subject) {
                ...
                // TODO: depth here?
                assertion.subjectType.inspect(assertion.subject, 0, subjectOutput);
            }
            useSubject = false;
        }
