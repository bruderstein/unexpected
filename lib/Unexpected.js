...
function thenResult(resolve, reject) {
    ...
    if (context.promise) {
        ...
        return context.promise.then(function () {
            resolve(context.subject);
        }, reject);
    }
    return resolve(this._context.subject);
}

function forwardFlags() {
    this._context.forwardFlags = this._context.parentFlags;
    return this;
}
...
function executeExpect(unexpected, context, subject, testDescriptionString, args) {
    ...
    var wrappedExpect = function (subject, testDescriptionString) {
        ...
        return wrappedExpect.callInNestedContext(function () {
            return executeExpect(that, context.child(), subject, testDescriptionString, args);
        });
    };
...
function wrapAssertions(instance, name) {
    ...
    var wrapped = function () {
        ...
        if (this._context.promise) {
            this._context.promise = this._context.promise.then(function () {
                return callAssertion.call(that, instance, that._context.expectContext, name, args);
            });
            return this;
        }

        var result;
        result = callAssertion.call(that, instance, that._context.expectContext, name, args);
        if (isPromiseLike(result)) {
            this._context.promise = result;
        }
        return this;
    };
    return wrapped;
...
function callAssertion(unexpected, context, name, args) {
    var that = this;
    var subjectType = this._context.subjectType;
    var subject = this._context.subject;
    var assertionRule = unexpected.lookupMethodAssertionRule(this._context.subject, name, args, false);
    if (!assertionRule) {
    ...
    this._context.calledAssertions.push(calledAssertionEntry);

    var testDescriptionString = assertionRule.testDescriptionString;
    var flags = extend({}, assertionRule.flags);
    if (this._context.forwardFlags) {
        var forwardFlags = this._context.forwardFlags;
        Object.keys(this._context.forwardFlags).forEach(function (flagName) {
    ...
    var chainingContext = this._context;
    ...
    wrappedExpect.standardErrorMessage = function (output, options) {
        return chainingStandardErrorMessage.call(unexpected, that._context, output, options);
    };

    wrappedExpect.getChainingContext = function () {
...
function splitShiftedAssertions(wrappedExpect, calledAssertions, error) {
...
Unexpected.prototype._expectChained = function (context, subject, parentFlags) {
    ...
    const rootWithState = Object.create(this.assertionRoot, {
        // TODO: rename this as _chainingContext
        _context: {
            value: {
                subject: subject,
...
function createStandardErrorMessageFromChain(output, calledAssertions, options) {
...
// TODO: This is only for chaining, and actually belongs somewhere else
function chainingStandardErrorMessage(context, output, options) {
