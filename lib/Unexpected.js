...
Unexpected.prototype.expect = function expect(subject, testDescriptionString) {
    ...
    function executeExpect(executionContext, subject, testDescriptionString, args) {
        ...
        var callInNestedContext = function (executionContext, callback) {
            try {
                ...
                if (isPendingPromise(result)) {
                    ...
                    result = result.then(undefined, function (e) {
                        if (e && e._isUnexpected) {
                            throw new UnexpectedError(wrappedExpect, e);
                        }
                        throw e;
                    });
        ...
        var wrappedExpect = function wrappedExpect() {
            ...
            return callInNestedContext(executionContext, function () {
        ...
        wrappedExpect.fail = function () {
            var args = arguments;
            callInNestedContext(executionContext, function () {
        ...
        wrappedExpect.standardErrorMessage = function (output, options) {
            ...
            if (options && options.compact) {
                options.compactSubject = function (output) {
                    var subjectType = wrappedExpect.findTypeOf(wrappedExpect.subject);
                    output.jsFunctionName(subjectType.name);
                };
            }

            return createStandardErrorMessage(output, wrappedExpect.subjectOutput, wrappedExpect.testDescription, wrappedExpect.argsOutput, options);
        };
        ...
        wrappedExpect.shift = function (subject, assertionIndex) {
            ...
            var args = wrappedExpect.args.slice(0, assertionIndex);
            var rest = wrappedExpect.args.slice(assertionIndex);
            var nextArgumentType = wrappedExpect.findTypeOf(rest[0]);
            wrappedExpect.argsOutput = function (output) {
